    ...
    def __init__(self, sequence_length, num_classes,
        ...
        with tf.name_scope("bi-lstm"):
            ...
            (output_fw, output_bw), states = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell,
                                                                               ...
                                                                               dtype=tf.float32)
            Z = tf.concat([output_fw, output_bw], axis=2)
            fw_cell1 = tf.nn.rnn_cell.GRUCell(hidden_size)
            fw_cell1 = tf.nn.rnn_cell.DropoutWrapper(fw_cell1, output_keep_prob=0.8)
            bw_cell1 = tf.nn.rnn_cell.GRUCell(hidden_size)
            bw_cell1 = tf.nn.rnn_cell.DropoutWrapper(bw_cell1, output_keep_prob=0.8)
            (output_fw1, output_bw1), states = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell1,
                                                                               cell_bw=bw_cell1,
                                                                               inputs=Z,
                                                                               sequence_length=text_length,
                                                                               dtype=tf.float32)
            with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                T = tf.concat([output_fw1, output_bw1], axis=2)
                fw_cell11 = tf.nn.rnn_cell.GRUCell(hidden_size)
                fw_cell11= tf.nn.rnn_cell.DropoutWrapper(fw_cell11, output_keep_prob=0.95)

                bw_cell11 = tf.nn.rnn_cell.GRUCell(hidden_size)
                bw_cell11 = tf.nn.rnn_cell.DropoutWrapper(bw_cell11, output_keep_prob=0.95)
                (self.output_fw, self.output_bw), states = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell11,
                                                                                           cell_bw=bw_cell11,
                                                                                           inputs=T,
                                                                                           sequence_length=text_length,
                                                                                           dtype=tf.float32)
