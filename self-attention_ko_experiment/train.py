...
# Data loading params
tf.flags.DEFINE_string("train_dir", "data/train.sent_data.txt", "Path of train data")
tf.flags.DEFINE_float("dev_sample_percentage", .1, "Percentage of the training data to use for validation")
tf.flags.DEFINE_integer("max_sentence_length", 70, "Max sentence length in train/test data")
...
tf.flags.DEFINE_integer("hidden_size", 500, "Size of LSTM hidden layer")
...
tf.flags.DEFINE_integer("evaluate_every", 600, "Evaluate model on dev set after this many steps")
...
def train():
    ...
    vocab_processor = tf.contrib.learn.preprocessing.VocabularyProcessor(FLAGS.max_sentence_length, tokenizer_fn=data_helpers.my_func)
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # Define Training procedure & RMSProp Optimizer
            ...
            utc = arrow.now()
            res_time = utc.format().split()
            res = res_time[0] + "_" + res_time[1].split('+')[0]

            out_dir_name = res.replace('-','_')
            out_dir_name = out_dir_name.replace(':','_')

            out_dir = os.path.abspath(os.path.join(os.path.curdir, "runs", out_dir_name))
            print("Writing to {}\n".format(out_dir))
            ...
            if FLAGS.word2vec:
                ...
                with open(FLAGS.word2vec, "rb") as f:
                    ...
                    for line in range(vocab_size):
                        word_to_vec = f.readline().decode('utf-8')
                        word_to_vec = word_to_vec.split()
                        word = word_to_vec[0]
                        res_vec_list = np.array(word_to_vec[1:], dtype='float32')
                        idx = vocab_processor.vocabulary_.get(word)
                        if idx != 0:
                            initW[idx] = res_vec_list
            ...
            for batch in batches:
                ...
                if step % FLAGS.evaluate_every == 0:
                    ...
                    if (accuracy_dev / cnt) > 0.769:
                        print("-Eval-\nstep {}, loss {:g}, acc {:g}\n".format(step, loss_dev / cnt, accuracy_dev / cnt))
